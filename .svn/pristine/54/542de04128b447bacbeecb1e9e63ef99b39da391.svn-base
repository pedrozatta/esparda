var app = angular.module('esparda-core', [ 'ngRoute' ]);
		...
		.config([
				...
				function($routeProvider, $httpProvider) {
					...
					$routeProvider.when('/', {
						...
						templateUrl : 'view/restclient.html',
...
app.controller('RestClientCtrl', function($rootScope, $scope, $http, $q) {
	...
	$scope.enviar = function() {
		...
		case "post":
		case "put":
			$http[data.metodo](data.url, data.json).success(sucessFunction)
					.error(errorFunction);
			break;

		default:
			$http[data.metodo](data.url).success(sucessFunction).error(
					errorFunction);
			break;
		}
...
app.controller('MonitorCtrl',
		function($rootScope, $scope, $http, $q) {
			...
			var sucessFunction = function(data) {
				...
				if (angular.isArray(data)) {
					for (i = 0; i < data.length; i++) {
						data[i].primeira = new Date(data[i].primeira);
						data[i].ultima = new Date(data[i].ultima);
					}
				}
				$scope.$emit('esp-event-message:info');
			};
			...
			$scope.atualizar = function() {
				$http.get('monitor/all').success(sucessFunction).error(
						errorFunction);
			}
			$scope.atualizar();

			$scope.limpar = function() {
				$http.get('monitor/clean').success(sucessFunction).error(
						errorFunction);
			}
